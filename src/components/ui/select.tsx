...
function SelectGroupScrollable({
  className,
  label,
  placeholder,
  labelStyle,
  ...
  return (
    <Select>
      ...
      >
        <div className="w-full items-start justify-start flex-col flex bg-white">
          {label && (
            <label
              htmlFor={name}
              className={cn(
      ...
      <SelectContent className="z-[9999]">
        {group_options.map(({ group_label, options }, index) => (
          <SelectGroup key={`${name}-${index}-${group_label}-`}>
            {options.map(({ label = "", value = "" }, index) => (
              <SelectItem
                key={`${name}-${index}-${group_label}-${label}-${value}`}
                value={value}
              >
                {label}
              </SelectItem>
